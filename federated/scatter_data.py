...
def copy_and_extract(dataset: str, local_storage: str, file: str) -> None:
    ...
    destination_directory = f'{local_storage}/IIFL-LN/datasets/{dataset}/'
    shutil.copy(f'datasets/{dataset}/{file}', destination_directory)
    tar_file_path = f'{local_storage}/IIFL-LN/datasets/{dataset}/{file}'
...
def transfer_local_dataset(dataset: str, node_rank: int) -> None:
    ...
    print(f"Node of rank {node_rank}."
          f" Datasets directory files: {os.listdir(f'{slurm_tmpdir}/IIFL-LN/datasets/')}"
          f" Local dataset files: {os.listdir(f'{slurm_tmpdir}/IIFL-LN/datasets/{dataset}/')}")
