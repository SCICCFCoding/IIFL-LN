...
from immune.detector import ImmuneDetector
...
class Server(Node):
    ...
    def __init__(self, server_opt: str = 'fedavg', serverlr: float = 1., tau: float = None, beta: float = None) -> None:
        ...
        self.immune_detector = ImmuneDetector(threshold=getattr(self, 'detection_threshold', 0.8),cosine_threshold=0.5 )
    ...
    def aggregate(self, state_dicts_encrypted: List[Tuple[bytes, bytes, bytes, int]]) -> None:
        ...
        if self.use_immune_detection and hasattr(self, 'immune_detector') and self.__clients_public_keys and len(updates) > 0:
        # if self.__clients_public_keys and len(updates) > 0:
            print(f"[IMMUNE DEBUG] len(updates)={len(updates)}; clients_public_keys={list(self.__clients_public_keys.keys())}")


            client_ranks = list(self.__clients_public_keys.keys())  
            all_clients = client_ranks  
            print(f"[IMMUNE DEBUG] all_clients={all_clients}")

            client_params = {rank: update for rank, update in zip(client_ranks, updates)}

            trusted_clients = self.trusted_clients if hasattr(self, 'trusted_clients') else all_clients 
            # trusted_clients = self.trusted_clients if self.trusted_clients else all_clients
            filtered_params, normal_clients = self.immune_detector.detect(trusted_clients, client_params, all_clients)
            normal_indices = [client_ranks.index(cid) for cid in normal_clients]
...
class Client(Node):
    ...
    def __init__(self, rank: int, attack_type: int = 0) -> None:
    ...
    def train(self, nrounds: int, kround: int, epochs: int, architecture: str, data: str, bsz_train: int, imgsz: int,
        ...
        # os.makedirs(f'{saving_path}/run/train-client{self.rank}/weights/', exist_ok=True)
        os.makedirs(f'{saving_path}/run/train-client{self.rank}/weights/', exist_ok=True)
        if kround == 0:
            ...
            try:
                ...
                print(f"Client {self.rank}: Error during training for round {kround}: {e}")
        else:
        ...
        if os.path.exists(end_weights):
            ...
            original_update = copy.deepcopy(delta_it)
            ...
            print(f"Client {self.rank}: Training did not produce weights file. Using current model state.")
            if kround == 0:
                self.post_init_update(data, cfg, hyp, imgsz)
            w_t = self._ckpt['model'].half().state_dict()
            ...
            delta_it = copy.deepcopy(w_t)
            for key in delta_it.keys():
                delta_it[key] = torch.zeros_like(delta_it[key])
            self.__update = delta_it